function rainbow_line
	set -f ONE_FIFTH_COLUMNS (math --scale=0 $COLUMNS / 5)
	set -f COLOR_DELTA (math --base="hex" --scale=0 0xFF / $ONE_FIFTH_COLUMNS)
	set -f RED 0xFF
	set -f GREEN 0x00
	set -f BLUE 0x00

	for COL in (seq 1 $ONE_FIFTH_COLUMNS)
		set -f GREEN (math --base="hex" --scale=0 min 0xFF,(math --base="hex" --scale=0 $GREEN + $COLOR_DELTA))
		set_color --background (printf '%02x' $RED)(printf '%02x' $GREEN)(printf '%02x' $BLUE); echo -n " "
	end
	...
	for COL in (seq (math 4 "*" $ONE_FIFTH_COLUMNS) (math $COLUMNS - 1))
